...
const History = () => {
  ...
  const [medBuddyItems, setMedBuddyItems] = useState<
    (MedBuddyHistory | AppointmentHistory)[]
  >([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (medsRes.data.success) {
        combinedMedBuddy.push(
          ...medsRes.data.medications.map((med: any) => ({
            type: "medication",
            id: med._id,
            medication: med.name,
            dosage: med.dosage,
      ...
      if (apptsRes.data.success) {
        combinedMedBuddy.push(
          ...apptsRes.data.appointments.map((appt: any) => ({
            type: "appointment",
            id: appt._id,
      ...
      if (chatsRes.data.success) {
        setChatHistory(
          chatsRes.data.chats.map((chat: any) => ({
            id: chat._id,
            title: chat.title,
      ...
      if (readingRes.data.success) {
        setReadingHistory(
          readingRes.data.history.map((log: any) => ({
            id: log._id,
            title: log.title,
  ...
  const handleDelete = async (
    ...
    // Type changed to generic string to handle sub-types
    e.stopPropagation();
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {/* {showTrash ? (
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={(e) => handleRestore(e, scan.id, "scans")}
                        >
                          <RefreshCcw className="w-4 h-4 text-green-500" />
                        </Button>
                      ) : (
                        <Button
                          ...
                          variant="ghost"
                          className="opacity-0 group-hover:opacity-100 transition-opacity"
                          onClick={(e) => handleDelete(e, scan.id, "scans")}
                        >
                          <Trash2 className="w-4 h-4 text-destructive" />
                        </Button>
                      )} */}
                ...
                medBuddyItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-foreground">
                        {item.type === "medication"
                          ? (item as MedBuddyHistory).medication
                          : `Appt: ${(item as AppointmentHistory).doctorName}`}
                      </h3>
                      <div className="flex items-center gap-3 mt-1">
              ...
              {activeTab === "scans" && scanHistory.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  No scan history found
                </div>
              )}
              {activeTab === "medbuddy" && medBuddyItems.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  No history found
                </div>
              )}
